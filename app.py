...
def get_flow():
    ...
    print(f"DEBUG: client_config keys={list(client_config.keys())}")
...
def login():
    ...
    try:
        ...
        print(f"DEBUG: authorization_url 생성됨")
        print(f"DEBUG: state={state}")
        
        session['oauth_state'] = state
        
        print("INFO: Google 로그인 페이지로 리다이렉트")
        return redirect(authorization_url)
    except Exception as e:
        print(f"ERROR: /auth/login 오류: {e}")
        import traceback
        traceback.print_exc()
...
def callback():
    """Google OAuth 콜백"""
    print("INFO: Google OAuth 콜백 수신")
    
    try:
        error = request.args.get('error')
        if error:
            print(f"ERROR: OAuth 오류: {error}")
            return jsonify({"status": "error", "message": f"OAuth error: {error}"}), 400
        
        code = request.args.get('code')
        print(f"DEBUG: authorization_code 수신")
        
        if not code:
            print("ERROR: authorization_code 없음")
        ...
        print(f"INFO: Token 획득 완료")
        print(f"DEBUG: refresh_token 존재={credentials.refresh_token is not None}")
        ...
        print(f"ERROR: /auth/callback 오류: {e}")
        import traceback
        traceback.print_exc()
        return jsonify({"status": "error", "message": str(e)}), 500
